            ...
            if (Enum.TryParse<ExtensionStatus>(value, true, out var status))
            {
                switch (status)
                {
                    case ExtensionStatus.Running:
                        builder
                            .Append("<button type='button' class='play btn btn-sm btn-light' disabled><i class='fas fa-play'></i></button>&nbsp;")
                    ...
                    case ExtensionStatus.Stopped:
                        builder
                            ...
                            .Append("<button type='button' class='pause btn btn-sm btn-light' disabled><i class='fas fa-stop'></i></button>");
                        break;
                }
            }
